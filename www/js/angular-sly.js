...
angularSly.directive('slyVertical', function () {
  return {
    ...
    link: function (scope, el, attrs) {
      ...
      // Merge parts into options object for sly argument
      var options = $.extend({}, defaultOptions, defaultControls, scope.$eval(attrs.slyOptions));
      var callback = scope.$eval(attrs.slyCallback) || {};
      // Call Sly on frame
...
angularSly.directive('slyVerticalRepeat', [
  ...
  function ($timeout) {
    return {
      ...
      link: function (scope, el, attrs) {
          console.log("runnning link");
        if (scope.$parent.$last === true) {
            console.log("beforeTimoue");
          $timeout(function () {
            ...
            // Merge parts into options object for sly argument
            var options = $.extend({}, defaultOptions, defaultControls, scope.slyOptions|| {});
            var callback = scope.slyCallback || {};
            // Call Sly on frame
              console.log("hello");
              console.log(options);
                  scope.slyVerticalRepeat=new Sly(frame, options ,callback).init(); 
          });
        }
      }
    };
